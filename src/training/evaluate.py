...
The val split is reconstructed to MATCH training exactly:
  enumerate_patients(roots) -> filter_paired_patients (diffusion needs NAC+AC)
  -> make_patient_split(n, val_fraction, seed).
So you MUST pass the same ``--val_fraction`` and ``--seed`` the training run used
(GUI default was val_fraction=0.1, seed=42).
...
from src.training.data import filter_paired_patients, load_patient_by_path, make_patient_split
...
METRIC_KEYS = ["psnr", "ssim", "nrmse", "mae", "rel_bias"]


def _resolve_candidates(roots, mode, val_fraction, seed):
    """Return the ordered candidate patient list to evaluate.

    mode 'val' : reconstruct the training by-patient val split (paired only).
                 NOTE: only valid if the dataset has not changed since training
                 (if studies were added later the indices no longer line up).
    mode 'all' : return every enumerated patient (unfiltered); the eval loop
                 lazily skips the unpaired ones and stops at --max_patients, so
                 the expensive pairing scan only touches what it needs.
    """
    ...
    _, val_idx = make_patient_split(len(paired), val_fraction, seed)
    return [paired[i] for i in val_idx], len(paired)
...
def _eval_diff2d(args, device, candidates):
    ...
    for pi, patient in enumerate(candidates):
        ...
        if slice_metrics:
            agg = {k: float(np.mean([m[k] for m in slice_metrics])) for k in METRIC_KEYS}
...
def main():
    ...
    parser.add_argument("--mode", choices=["val", "all"], default="all",
                        help="'val' reproduces the training val split (only valid if the dataset is unchanged "
                             "since training); 'all' samples paired patients from the current dataset (default).")
    parser.add_argument("--max_patients", type=int, default=20, help="Cap on patients evaluated (0 = no cap)")
    parser.add_argument("--val_fraction", type=float, default=0.1, help="'val' mode only: MUST match training (GUI default 0.1)")
    parser.add_argument("--seed", type=int, default=42, help="'val' mode only: MUST match training (default 42)")
    ...
    candidates, n_total = _resolve_candidates(args.data_dir, args.mode, args.val_fraction, args.seed)
    if args.mode == "all":
    ...
    print(f"{n_total} {'paired' if args.mode=='val' else 'enumerated'} -> {len(candidates)} candidates "
          f"(cap {args.max_patients})")
    ...
    result = {
        ...
        "note": "normalized-intensity space; rel_bias is a proxy for SUV mean bias",
        "summary": summary,
    ...
    print("\n=== Val-set summary (mean +/- SD over patients) ===")
    for k in METRIC_KEYS:
        if k in summary:
