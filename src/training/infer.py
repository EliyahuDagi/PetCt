...
def _write_outputs(out_dir, pred, gt, vols):
    out = Path(out_dir)
...
def run_ae2d(args, device):
    ...
    recon = ae_decode(ae, ae_encode(ae, img))
    return recon[0, 0], img[0, 0], vols
...
def run_ae3d(args, device):
    ...
    recon = ae3d_decode(ae, ae3d_encode(ae, vol))
...
def main():
    ...
    pred, gt, vols = RUNNERS[args.task](args, device)
    out_dir = _write_outputs(args.out, pred, gt, vols)
    print("Inference complete. Wrote pred.npy / gt.npy / meta.json to %s" % out_dir)
