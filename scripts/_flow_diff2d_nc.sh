...
echo ">>> STAGE_START flow_nc_train $(date '+%H:%M:%S')"
"$PY" -m src.training.train.launcher diff2d \
  ...
  --cache_size 280 \
  > outputs/diff2d_flow_nc.log 2>&1
rc=$?; echo ">>> STAGE_END flow_nc_train rc=$rc $(date '+%H:%M:%S')"
...
for S in 16 32; do
  ...
  "$PY" -m src.training.evaluate --task diff2d \
    ...
    --out outputs/eval/diff2d_flow_nc/test_s$S.json > outputs/eval_flow_nc_s$S.log 2>&1
