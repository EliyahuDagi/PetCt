#!/usr/bin/env bash
# Retrain diff2d with classifier-free guidance (cond_dropout_prob=0.1), reusing the
# existing ae2d. Saved to outputs/diff2d_cfg (baseline outputs/diff2d preserved).
# Then evaluate on the held-out TEST split across a guidance-scale sweep.
set -u
...
echo ">>> STAGE_START diff2d_cfg_train $(date '+%H:%M:%S')"
...
rc=$?; echo ">>> STAGE_END diff2d_cfg_train rc=$rc $(date '+%H:%M:%S')"
if [ $rc -ne 0 ]; then echo ">>> ABORT train"; exit $rc; fi

# ---- Guidance sweep on the TEST split ----
