#!/usr/bin/env bash
# Retrain the NAC->AC flow bridge (flow + perceptual_weight 0.1) on the freshly
# retrained 2D AE (outputs/ae2d_p/best.pt). Fresh dir so the previous flow+perc run
# on the old AE (outputs/diff2d_flow_perc) is kept for comparison.
set -u
